...
const TOOLS = {
  ...
  'php_modernize':          { dept:'PHP & SFTP 部署', title:'PHP 舊語法確定性升級', desc:'用 token_get_all 詞法分析做 6 條語法等價機械轉換：移除結尾 ?>、$str{0}→$str[0]、PHP4 同名建構子→__construct（有 namespace 自動略過）、var→public、define 裸常數加引號、__autoload 改名+spl_autoload_register。每檔改完容器內 php -l 過才寫回；mysql_*/ereg/each 等需判斷的不自動改只列殘留數交 LLM。預設 apply:false 預覽，確認才寫，自動跳過 vendor/lib/plugin。', usage:'php_modernize {path:"proj/cls", apply:false, container:"dev-php84"}', tools:[] },
  ...
  'docker_cp':              { dept:'PHP & SFTP 部署', title:'本機 Docker container 檔案拷貝', desc:'把本機檔案放進 docker container 或從 container 拉出來。適用 Downloads/ 等非容器掛載點的檔案要進容器（如 SQL dump 進 DB 容器）。內建 basePath 白名單 + container 名 + container_path regex 防注入。遠端機器仍用 ssh_exec。', usage:'docker_cp {direction:"to_container", container:"dev-php84", local_path:"D:\\\\xxx\\\\xxx.sql", container_path:"/tmp/xxx.sql"}', tools:[] },
  ...
  'find_dead_symbols':      { dept:'PHP 分析', title:'零引用死碼掃描', desc:'掃整個專案找零引用的死碼候選：用符號索引列出所有 class/method/function 定義，再用全專案呼叫記錄反查，列出沒有任何 new/static/方法呼叫/extends/implements 指向的符號。分三層信心（函式高、方法中、類別低），偵測 call_user_func 等動態呼叫時降信心提醒。取代逐一 find_usages。刪除前須複查字串 callback/動態類名/型別提示。', usage:'find_dead_symbols {project:"...", types:["function","method","class"]}', tools:[] },
  ...
  'csv_recompute_audit':    { dept:'檔案系統 & 資料庫', title:'Baseline CSV vs PHP 重算 Diff 報告', desc:'對照 baseline CSV 跑 PHP class::method 重算後輸出 diff 報告 CSV。情境：計算類 service 對齊 Sheet baseline，避免每次改算法就重跑 GSheet quota。嚴格字串相等比對（不設 tolerance），支援 docker container 模式。', usage:'csv_recompute_audit {project:"PG_myproject", baseline_csv:"_harness/baseline.csv", output_csv:"_harness/audit.csv", class:"MyService", method:"compute", args_from:["case_json"], mapping:{F825:"result.F825"}, container:"dev-php84"}', tools:[] },
  ...
  'gsheet_get_values':      { dept:'檔案系統 & 資料庫', title:'Google Sheet 輕量 batch get', desc:'純讀多個 range 的值，免寫入、免 recalc polling。value_render 可選 UNFORMATTED_VALUE（原始數值，預設）/ FORMATTED_VALUE（顯示字串）/ FORMULA（公式）。取代估價對齊 harness 在 PHP 內手刻 JWT + curl 打 Sheets API 讀 live Sheet。', usage:'gsheet_get_values {spreadsheet_id:"...", read_range:["web!A1:Z200","data!B5"], value_render:"FORMATTED_VALUE"}', tools:[] },

  'gsheet_set_values':      { dept:'檔案系統 & 資料庫', title:'Google Sheet 輕量 batch update', desc:'純寫入多個 range，不等公式重算、不回讀（要寫後 polling 回讀用 fetch_with_state）。取代估價對齊 harness 在 PHP 內手刻 JWT + curl 寫 live Sheet。⚠️ 寫線上 Sheet 屬破壞性操作，空字串會清 cell 並可能清掉 data validation。', usage:'gsheet_set_values {spreadsheet_id:"...", write_data:[{range:"web!A3", values:"X"},{range:"web!B1:C1", values:[["a","b"]]}]}', tools:[] },

  'list_memory_triggers':   { dept:'Memory & Hook', title:'列 memory triggers 狀態', desc:'掃描專案 memory dir 所有 .md 的 frontmatter triggers 欄位，列出哪些已設、哪些缺。配合 memory-auto-recall hook 用。', usage:'list_memory_triggers {project_id:"d--Project-MyProject", only_missing:false}', tools:[] },
  'memory_add_triggers':    { dept:'Memory & Hook', title:'設定 memory triggers', desc:'為單筆 memory frontmatter 設定 / 更新 triggers（tools / path_patterns / prompt_keywords / reinject_after_tool_calls）。merge_mode 控制覆蓋或合併。', usage:'memory_add_triggers {memory_file:"d--Project-X/memory/foo.md", triggers:{tools:["Edit"], path_patterns:["{module}"], prompt_keywords:["sheet"], reinject_after_tool_calls:25}}', tools:[] },
  ...
  'session_search':         { dept:'系統工具', title:'跨 session 搜歷史對話', desc:'全文搜 ~/.claude/projects 下所有歷史對話 JSONL：用檔名 / 錯誤訊息 / 功能關鍵字，查「以前是不是處理過、結論是什麼」。卡關或反覆改同檔時即時呼叫，斷掉換場重踩同個坑、省 token 空轉。', usage:'session_search {keyword:"MyService.php", days:30, project:"myproject"}', tools:[] },
  'session_recall':         { dept:'系統工具', title:'回顧某一場對話', desc:'回顧過往某一場做了什麼、做完沒、卡在哪、動過哪些檔、跑過哪些 SQL/PHP、最後結論。接手未完成工作或查上一場進度用。唯讀。', usage:'session_recall {project:"myproject", selector:"prev"}', tools:[] },
};
