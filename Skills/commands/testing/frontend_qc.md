...
- **設計稿比對**：必須先截圖設計稿並輸出提取值，才能寫比對結論。無設計稿輸出 = 該項 [UNVERIFIED] = HIGH。
- **規格書比對**：必須先輸出規格書原文，才能寫比對結論。無規格書引用 = 該項 [UNVERIFIED] = HIGH。
- **禁止憑印象**：所有 Bug 描述的「期望」欄位必須引用設計稿截圖路徑或規格書原文片段，否則無效。
- **無法讀取來源時**：立即標記 [SKIP-無法讀取設計稿] 或 [SKIP-無法讀取規格書]，不得假設內容繼續比對。
