...
> **舉證義務與截圖規則**：遵循 `_project_qc/global_rules.md` 中的「舉證義務」與「截圖全域規則」。
> 核心：無設計稿截圖/規格書引文 = [UNVERIFIED] = HIGH；無法讀取來源 = [SKIP]，不得假設內容繼續比對。
...
⚠️ 強制兩段式舉證：**設計稿先讀先輸出，禁止在此之前寫任何比對結論。**

**Step 1【先讀設計稿 — 不可跳過】**

```
1. Read 設計稿圖片（{page}_design.png 或 XD/Figma 截圖）
   → 若找不到設計稿：標記 [SKIP-無設計稿] 並通知使用者，不繼續 2a
2. 從設計稿提取並輸出：

=== 設計稿：{頁面名稱} ===
截圖路徑：screenshots/qc/{page}_design.png
主色：#xxxxxx
輔色：#xxxxxx
按鈕色：#xxxxxx（用 browser_evaluate getComputedStyle 取樣）
標題字型：xxx / 大小：xxpx
主要 Column 數：N
區塊順序：Header → Banner → {區塊1} → {區塊2} → Footer
特殊元素：（輪播箭頭/浮動按鈕/固定Header等）
...
2. browser_take_screenshot → screenshots/qc/{page}_live.png
3. browser_evaluate → 取樣關鍵元素 CSS 值：
   document.querySelector('header').style 或 getComputedStyle(...)
   輸出實際值（對應 Step 1 的每個項目）：
...
**Step 3【逐項比對 — 必須引用上方兩段數值】**

```
對每個項目：設計稿值 vs 實際值 → 差異 → 嚴重度

色彩偏差 > ±5%   → [VISUAL-HIGH]
Column 數不同     → [VISUAL-HIGH]
區塊缺失/順序錯   → [VISUAL-HIGH]
間距/對齊偏差     → [VISUAL-MED]
字型大小偏差      → [VISUAL-MED]
陰影/圓角微差     → [VISUAL-LOW]
```
...
⚠️ 強制兩段式舉證：**規格書原文先輸出，禁止在此之前寫任何比對結論。**

**Step 1【先讀規格書 — 不可跳過】**

```
1. Read / Grep 規格書索引，找到該頁面的欄位定義
   → 若找不到該頁面：標記 [SKIP-規格書無此頁面]，不繼續 2b
2. 輸出規格書原文摘要：

=== 規格書：{頁面名稱} ===
欄位清單：
  1. {欄位名} | {類型 input/select/textarea/radio} | {必填/選填} | {預設值/選項}
  2. ...
按鈕清單：
  - {按鈕名} | {行為描述}
業務規則：
...
```
browser_snapshot → 取得實際 DOM 結構
輸出實際欄位清單（對應規格書順序）：

=== 實際：{頁面名稱} ===
欄位清單：
  1. {欄位名} | {type/tag} | {required?} | {placeholder/options}
  2. ...
按鈕清單：
  - {按鈕名} | {onclick/href}
```

**Step 3【逐條比對 — 必須引用上方兩段】**

```
對每條規格書定義：
   - 規格書有、DOM 沒有   → [FIELD-MISSING]
   - DOM 有、規格書沒有   → [FIELD-EXTRA]
   - 欄位名稱不符         → [FIELD-LABEL]（規格：xxx 實際：yyy）
   - 欄位類型不符         → [FIELD-TYPE]（規格：select 實際：input）
   - 欄位順序不符         → [FIELD-ORDER]
   - 下拉選項不符         → [FIELD-OPTIONS]（規格：A/B/C 實際：A/B）
   - 業務規則未實作        → [FUNC-HIGH]（引用規格原文）
```
...
讀取 `d:\Develop\MCP_NodeServer\Skills\commands\testing\_frontend_qc\report_template.md` 作為報告格式，儲存至 `{ProjectFolder}/reports/frontend/qc_report_{date}.md`。

---

### 步驟 5：PM 迭代循環（自動反覆直到全部通過）

讀取 `d:\Develop\MCP_NodeServer\Skills\commands\testing\_frontend_qc\pm_loop.md`，依 PM 迭代循環步驟執行（含工程師任務單格式）。
...
- **PM 自動循環**：報告產出後自動進入迭代循環，產出修正任務單並等工程師確認，複驗直到所有 Bug 關閉，不需手動重跑 `/frontend_qc`
