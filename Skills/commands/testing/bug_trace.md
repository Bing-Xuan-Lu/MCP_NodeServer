...
在動手 trace 之前，先判斷使用者抱怨屬於哪個層級。**截圖只證明「現象出現」，不證明使用者在意的層級。**

| 層級 | 定義 | 典型用語 |
|------|------|------|
| **layout** | 位置/樣式/RWD/視覺呈現 | 「跑版」「位置歪」「字太小」「擠在一起」「破版」 |
| **trigger** | 不該出現卻出現 / 該出現沒出現 / 出現時機錯 | 「為什麼會跳這個」「點了沒彈窗」「重複出現」 |
| **data** | 顯示資料錯誤 / 數值不對 / 欄位錯位 | 「金額算錯」「顯示空白」「跟 DB 對不上」 |
| **interaction** | 操作沒反應 / 反應錯 / 流程卡住 | 「按了沒反應」「送出失敗」「沒跳轉」 |

**判定原則：**

1. 使用者文字 + 截圖能**明確判定**單一層級 → 直接進步驟 1，並在報告開頭聲明 `本次追蹤層級：{level}`
2. 訊息模糊（「壞了」「不對」「有問題」「跑版」+ 附圖但沒寫具體哪裡）→ **必須先反問 1 句**確認，禁止直接跳 Playwright / Grep：
   > 「想先確認一下：你在意的是 (A) **位置/樣式**跑版 / (B) **不該出現**這個元素 / (C) **顯示資料**錯 / (D) **操作沒反應**？方便我聚焦根因。」
3. 多層級可能都有問題 → 請使用者排優先級，不要自己挑

**為什麼這條是鐵律：** 同一張「popup 位置歪」的截圖，使用者可能在意：
- (layout) popup 樣式跑掉 → 改 CSS
- (trigger) popup 不該出現 → 找觸發來源砍掉
- (data) popup 內資料錯 → 追資料來源

三個方向修法完全不同。先反問 1 句省 6+ 輪繞遠路。
