...
5. **CRUD 測試**：逐步測試 list / add / detail / del，用 `execute_sql_batch` 批次驗證多筆 DB 寫入
   ...
   - 不因單一模組失敗中斷整個測試批次
6. **前台串接驗證**（每個模組 CRUD 測完後立即做，見下方「步驟 5.5」）
7. **邊界矩陣測試**：對每個欄位系統性測試邊界條件與錯誤情境（見下方邊界矩陣）
8. **檔案測試**：若有上傳功能，用 `send_http_requests_batch` 批次測試多個上傳/下載端點
9. **清理**：刪除測試資料和測試腳本
10. **產出報告**：列出每個測試項目的結果、發現的問題、建議

## 步驟 5.5：前台串接驗證（每個模組 CRUD 測完後立即做）

後台新增成功後，**立即用 `send_http_request` GET 前台對應頁面**，驗證：

1. 剛新增的資料出現在前台列表（搜尋 HTML 中的測試值）
2. 點進詳細頁 → HTML 中包含所有填入的欄位值
3. 圖片欄位 → GET 圖片 URL → 驗 HTTP 200（不是 404）
4. 後台修改欄位值 → 再 GET 前台 → 驗值已更新
5. 後台刪除 → 再 GET 前台 → 驗已消失

**記錄格式**：

| 模組 | 操作 | 後台值 | 前台 URL | 前台是否正確 | 結果 |
|------|------|--------|---------|------------|------|

若前台無法用 GET 驗證（需要 JS 渲染），標記 [NEED-PLAYWRIGHT] 由 UI/UX 稽核員補驗。
