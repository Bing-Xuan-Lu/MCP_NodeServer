...
### 步驟 1b：Hook 健康度檢查（順帶執行）

從 `~/.claude/settings.json` 的 `hooks` 物件中提取所有 command 路徑：

```text
對每個 "command": "node \"C:/.../*.js\"" 提取 .js 絕對路徑
→ Bash: ls "{路徑}" 驗証是否存在
→ ❌ 缺失 → 在步驟 4 報告頂部顯示 ⚠️ BROKEN HOOK 警告
→ ✅ 全部存在 → 報告中顯示「Hook 健康度：✅ 全部正常」
```
