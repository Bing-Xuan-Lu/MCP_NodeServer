...
for f in ~/.claude/commands/*.md; do
  ...
  echo "$first" | grep -qE "^# /[a-z0-9_]+ —" || echo "❌ $(basename $f): $first"
done
...
- 第一行不是 `# /skill-name — 說明` 格式，且 `# /name —` 行不存在於檔案中
- 後果：`grep "^# /"` 掃描時完全隱形，skill_audit 偵測不到
- 常見原因：舊版 Skill 未按範本撰寫、手動建立未遵守格式
- **例外（非 D 類問題）**：
  - YAML frontmatter（`---` 開頭）— CLAUDE.md 明定為可選設計，`# /name —` 在 frontmatter 之後，仍可被掃到
  - 技能名稱含數字（如 `n8n_*`）— regex 需使用 `[a-z0-9_]+` 才能正確匹配
...
- Skill 執行步驟需要呼叫 MCP 工具，但沒有 `## 可用工具` 章節
- 後果：Claude 不清楚可用工具範圍，容易漏用 batch 版或更適合的工具，降低執行效率
- 判斷標準：步驟中若有「讀取」「掃描」「寫入」「執行」「驗證」等動作，幾乎必然需要工具章節
- 例外：純指引型 Skill（只輸出建議，不實際操作檔案/DB/HTTP）可省略工具章節

#### H — Token 膨脹（Skill 過長）

- 公開 Skill MD 超過 **400 行**或含有以下膨脹模式之一
- 後果：注入 prompt 時消耗大量 token，稀釋 Claude 對核心指令的注意力，降低執行品質
- **膨脹模式清單**：
  1. **內嵌生產程式碼**：完整 JS/Python 腳本直接寫在 MD 中（應抽到 `_{skill}/` 參考檔）
  2. **內嵌報告模板**：>50 行的 Markdown/HTML 報告範例（應抽到 `_{skill}/` 參考檔）
  3. **內嵌參考表格**：before/after 程式碼、版本差異表、欄位類型對照（應抽到 `_{skill}/` 參考檔）
  4. **規則重複陳述**：同一條規則在步驟中說一次、注意事項又說一次
  5. **冗長注意事項**：>20 行的「注意事項」段落，重複前文已述的約束
- **閾值**：
  - > 400 行 = ⚠️ 建議檢視
  - > 600 行 = 🔴 應立即拆分
- **修正方式**：將膨脹內容抽到 `Skills/commands/{dept}/_{skill}/` 子資料夾，主 Skill 改為引用（`讀取 _{skill}/xxx.md`）
- **已完成拆分的範例**：`project_qc` → `_project_qc/`（5 個參考檔）、`php_upgrade` → `_php_upgrade/`

```bash
# F — 找出誤放在公開部門的 _internal 命名 Skill
find {MCP_ROOT}/Skills/commands -name "*_internal*.md" | grep -v "/_internal/"

# G — 找出沒有「可用工具」章節的 Skill
for f in ~/.claude/commands/*.md; do
  grep -qE "^## (可用工具|工具)" "$f" || echo "⚠️  $(basename $f): 無工具章節"
done

# H — 找出超過 400 行的 Skill（Token 膨脹候選）
for f in ~/.claude/commands/*.md; do
  lines=$(wc -l < "$f" 2>/dev/null)
  if [ "$lines" -gt 600 ]; then
    echo "🔴 $(basename $f): ${lines} 行（應立即拆分）"
  elif [ "$lines" -gt 400 ]; then
    echo "⚠️  $(basename $f): ${lines} 行（建議檢視）"
  fi
done
```
...
1. 讀取目標 Skill 的 source 檔（`Skills/commands/{dept}/skill.md`），確認步驟中有哪些操作類型
2. 對照 MCP 工具清單，判斷需要哪些工具分組（檔案操作 / DB / 驗證 / 文件讀取等）
3. 在 `## 使用者輸入` 與 `## 執行步驟`（或 `## 背景`）之間插入 `## 可用工具` 章節
4. 使用 `apply_diff` 或 `Edit` 更新 source 檔
5. `cp` 更新後的檔案到 `~/.claude/commands/`
6. 多個 Skill 缺漏時，逐一詢問使用者確認再批次修正
