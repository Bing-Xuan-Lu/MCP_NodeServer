...
```
Grep pattern="require\('./tools/" path="index.js"
→ 提取所有 tools/*.js import 路徑
→ Bash ls 驗証每個模組檔案存在
→ ❌ 缺失 → 報告頂部顯示 ⚠️ MISSING MODULE
```

#### C. Skills 部署同步狀態

```
Bash: ls Skills/commands/**/*.md | wc -l   （來源數）
Bash: ls ~/.claude/commands/*.md | wc -l   （已部署數）
→ 差異 > 3 → 提示可能有未部署的 Skill（deploy-commands.bat 未執行）
```
...
| **8. 工具健康度** | 已設定的 hook/工具是否真的存在且能執行 | 步驟 0 的 Hard Check 結果 |
...
分項評分：
  ...
  安全防護      {X}/10  {說明}
  成本效率      {X}/10  {說明}
  工具健康度    {X}/10  {說明（步驟 0 結果，10=全通過，每個問題 -2 起）}
...
- 每季執行一次，追蹤成熟度趨勢
- **報告完成後，將本次分數追加寫入 `docs/audit_history.md`**（日期 + 8 分項 + 總分 + 備註）
