...
| 參數 | 說明 | 範例 |
|------|------|------|
| 專案資料夾 | 專案根目錄資料夾名稱 | `***_ERP` |
| .NET 專案名稱 | .NET 專案前綴 (影響 *.Web, *.Models, *.BLL 路徑) | `PNS` |
| PHP 資料夾 | PHP 專案資料夾名稱 | `***_ERP_PHP` |
...
4. **讀取 PHP 風格參考**：讀取 `{PHP資料夾}/adminControl/project/*.php` 作為風格規範
5. **查詢資料表結構**：用 `get_db_schema` 取得 MySQL 欄位資訊
6. **列出翻寫計畫**：告知使用者將產生哪些 PHP 檔案，等確認後再動手
7. **產生 PHP 程式碼**：輸出到 `{PHP資料夾}/adminControl/{模組}/` 和 `cls/model/`
