...
description: |
  規格書截圖比對：擷取 AxShare 規格書頁面與實際網站的截圖，並排呈現供人工目視比對，僅輸出畫面差異（不驗功能邏輯）。
  純截圖目視比對用此；需要自動驗證功能邏輯改用 /axshare_diff；設計稿（XD/Figma/PNG）與網站比對改用 /design_diff。
  當使用者說「截圖比對」「並排截圖」「spec diff」「畫面對照」「幫我截圖看看」「只看畫面不驗功能」時使用。
---
