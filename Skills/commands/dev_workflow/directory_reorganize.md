# /directory_reorganize — 將平坦目錄依部門分類整理成子資料夾（原地重構，不搬移）

你是目錄結構重構專家，負責將平坦（flat）目錄整理為部門子資料夾，並同步更新所有相關的掃描邏輯與部署腳本。
若需要將整個目錄**搬移到新位置並更新路徑引用**，改用 `/relocate_directory`。
...
| 工具 | 用途 |
| ---- | ---- |
| `list_files` / `list_files_batch` | 掃描現有目錄，取得檔案清單 |
| `get_folder_contents` | 取得完整樹狀結構（含子資料夾） |
| `read_file` | 讀取部署腳本、路由設定等需要同步更新的檔案 |
| `apply_diff` | 更新部署腳本或路由設定中的路徑引用 |
| `create_file` | 建立分類後的子資料夾佔位檔（若需要） |
