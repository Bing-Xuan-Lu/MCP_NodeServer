...
Agent (subagent_type=general-purpose):
  ...
  檢查項目：
    ...
    - [YAGNI] 是否新增了只被呼叫 1 次且 < 5 行的 helper（應內聯）
    - [YAGNI] 是否引入了不必要的套件（標準庫或 10 行內可自寫）
    - [YAGNI] 是否存在「以防萬一」的參數或永遠不會觸發的防禦性程式碼
```
...
**Refactor Pass**（在測試全過的綠燈狀態下），針對所有新增/修改的檔案逐一執行：

| 重構項目 | 門檻 | 動作 |
| --------- | ------ | ------ |
| 命名模糊（`data`/`result`/`temp`/`helper`） | 任何出現 | 替換為具體描述性名稱 |
| 函式過長 | > 50 行 | 提取子函式（僅限 3+ 呼叫點） |
| 重複邏輯 | 出現 3 次以上 | 提取共用函式 |
| 只呼叫 1 次的 helper（< 5 行） | 任何出現 | 內聯回呼叫處，消除不必要間接層 |
| 永遠不會觸發的防禦性程式碼 | 任何出現 | 移除（只保留外部輸入邊界的防禦） |
| 魔術數字 | 任何出現 | 提取為具名常數或移至 config |

每項重構後執行 `run_php_test` 確認行為不變。**重構後行數應減少或持平，若增加代表引入了不必要的間接層。**
