...
### 步驟 1.5：定義 UseCase 輸出契約

任務清單確認後，開發 SubAgent 動工前，先對每個 UseCase 輸出契約表——明確定義「通過是什麼」，讓開發與測試有共同基準。

對每個功能模組，依規格書輸出以下契約：

| UseCase | 成功輸出（型別 / 結構 / 必要欄位） | 失敗輸出（例外 / 錯誤碼 / 邊界行為） | 不測試範圍 |
|---------|----------------------------------|--------------------------------------|-----------|
| 新增 {module} | HTTP 200 + `success:true` + 新 id > 0；DB 有對應記錄且指定欄位非空 | 必填欄位空白 → HTTP 200 + `success:false` + 錯誤訊息；DB 無新記錄 | 通知、Log 機制 |
| 列表 {module} | HTTP 200；回傳陣列含 id / 主要欄位 | 無資料時回傳空陣列，非 500 | 分頁樣式 |
| 編輯 {module} | HTTP 200/302；DB 欄位值已更新 | id 不存在 → HTTP 200 + 錯誤提示 | 歷程記錄 |
| 刪除 {module} | HTTP 200/302；DB 該筆記錄不存在 | id 不存在 → 錯誤提示 | 軟刪除 vs 硬刪除（依規格） |

**契約確認規則：**
- 規格書有明確定義 → 照規格書填
- 規格書未提及 → 標記「待確認」並詢問使用者，不讓 SubAgent 自行推斷
- 複雜邏輯（計算公式、狀態機）→ 必須在此步驟完整定義，否則先跑 `/tdd` 再回來

> 契約確認後才進入步驟 2，SubAgent Prompt 必須附上對應模組的契約定義。
