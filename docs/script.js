...
const SKILLS = {
  ...
  'php_path_fix':          { dept:'PHP 開發部',     title:'修正 PHP 混合斜線路徑問題',        desc:'掃描 PHP 專案中所有 Windows 反斜線路徑寫法（如 include "\\Folder\\file.php"），統一修正為正斜線跨平台格式。',       usage:'/php_path_fix [掃描目錄路徑]',                             tools:['list_files','read_file','apply_diff'] },
  'dotnet_to_php':         { dept:'程式移植部',     title:'.NET C# → PHP 翻寫',             desc:'將 .NET（C#）程式碼翻譯為 PHP，保留業務邏輯，轉換語言特性（型別系統、LINQ、async/await、泛型等）。',               usage:'/dotnet_to_php [C#檔案路徑]',                            tools:['read_file','create_file','apply_diff','get_db_schema_batch'] },
  'php_crud_test':         { dept:'測試品管部',     title:'PHP CRUD 模組整合測試',              desc:'新功能開發完成後用於後端邏輯驗證：對 PHP 模組執行 CRUD 整合測試，逐步確認資料寫入與 DB 狀態一致性。',             usage:'/php_crud_test [專案資料夾] [PHP資料夾] [模組名稱]',                 tools:['send_http_request','send_http_requests_batch','execute_sql','execute_sql_batch','run_php_test','list_files_batch','read_files_batch','get_db_schema_batch'] },
  ...
  'sftp_deploy':           { dept:'部署維運部',     title:'本機 PHP 專案部署到遠端測試機',    desc:'透過 SFTP 將本機 PHP 目錄上傳到遠端伺服器，部署前確認目標目錄內容，部署後驗證遠端檔案結果。',                    usage:'/sftp_deploy [要部署的目錄或檔案(可省略)]',                              tools:['sftp_connect','sftp_upload','sftp_list'] },
  'sftp_pull':             { dept:'部署維運部',     title:'從遠端測試機拉取程式到本機',       desc:'透過 SFTP 將遠端伺服器的檔案或目錄下載到本機，適用於取回測試機改動、備份遠端資料或同步最新版本。',               usage:'/sftp_pull [遠端目錄或檔案]',                                tools:['sftp_connect','sftp_download','sftp_list','sftp_list_batch'] },
  ...
  'docker_relocate':       { dept:'Docker 維運部', title:'Docker Compose 開發環境搬遷',     desc:'將 Docker Compose 環境從一個路徑搬遷到另一個路徑，自動更新 Volume 掛載路徑與所有相關設定檔。',                    usage:'/docker_relocate [原始路徑] [新路徑]',                          tools:['read_file','apply_diff','create_file'] },
  ...
  'git_worktree':          { dept:'開發流程部',     title:'建立隔離 Git Worktree',           desc:'為需要並行開發的分支建立 Git Worktree，自動設定相依性與環境，讓多分支同時開發互不干擾。',                          usage:'/git_worktree [分支名稱]',                             tools:[] },
  ...
  'learn_claude_skill':    { dept:'Claude 維運部',  title:'從對話學習並建立 Skill',          desc:'當某個工作流程重複了 2-3 次、想固化為可重用指令時，從對話歷史提取模式、按規範撰寫 MD 並自動部署。',                 usage:'/learn_claude_skill [要學習的流程描述]',                       tools:['save_claude_skill','list_claude_skills'] },
  ...
  'verify':                { dept:'測試品管部',     title:'提交前六合一驗證',                 desc:'依序執行語法檢查、測試、Lint、git 狀態等六項驗證，產出 PASS/FAIL 報告，確認程式碼品質。', usage:'/verify [quick/full/pre-pr]', tools:['run_shell_command'] },
  ...
  'php_code_review':       { dept:'測試品管部',     title:'掃描未提交變更的安全問題',         desc:'對 git diff 找到的變更檔案進行安全與品質審查，攔截包含密碼外洩、SQL Injection 等嚴重問題的 commit。', usage:'/php_code_review [quick/full/pre-pr]', tools:['git_diff', 'git_status'] },
};
