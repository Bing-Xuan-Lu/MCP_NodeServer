...
const SKILLS = {
  ...
  'sadd':                  { dept:'開發流程部',     title:'規格書驅動逐任務派遣開發（含 DDD 架構規範）', desc:'有規格書後，以 SADD 模式建立任務清單，逐一派遣 SubAgent 執行並審查。步驟 0 內建 DDD 架構規範（四層目錄、命名規範、≤50行/≤200行/≤3層品質規則）；支援循序（DB→Model→Controller）與並行（獨立模組同步）兩種模式，防止上下文污染與缺陷累積。', usage:'/sadd [規格書路徑]', tools:[] },
  ...
  'sprint_plan':           { dept:'開發流程部',     title:'Tier-based 依賴層快速開發計畫', desc:'分析模組間的 DB 外鍵依賴關係，將工作拆解為 Tier 並行執行（同 Tier 並行、跨 Tier 循序），每模組附測試策略欄位，配合 /tdd + /sadd + /php_crud_test 完整工作流。', usage:'/sprint_plan {ProjectFolder} [規格來源]', tools:['list_files_batch','read_files_batch','get_db_schema_batch','create_file'] },
