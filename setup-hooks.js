...
// ── 3b. Generate env.js proxy in ~/.claude/ ─────────────
// Hooks use `import '../env.js'` which resolves to ~/.claude/env.js
// This proxy re-exports from the actual MCP_Server env.js
const envProxy = path.join(CLAUDE, 'env.js');
const mcpEnvUrl = `file:///${__dirname.replace(/\\/g, '/')}/env.js`;
const envContent = [
  '/**',
  ' * env.js — Auto-generated proxy for MCP_Server env.js',
  ` * Generated by setup-hooks.js at ${new Date().toISOString().slice(0, 19)}`,
  ' * Do not edit manually — re-run setup.ps1 or node setup-hooks.js to regenerate.',
  ' */',
  `export {`,
  `  HOME, MCP_ROOT, MCP_BASE_PATHS,`,
  `  CLAUDE_HOOK_DEBUG, CLAUDE_SLACK_WEBHOOK,`,
  `  CLAUDE_NOTIFY_ON_BLOCK, CLAUDE_TOKEN_FEEDBACK,`,
  `  CLAUDE_SUMMARY_INTERVAL,`,
  `} from '${mcpEnvUrl}';`,
  '',
].join('\n');
fs.writeFileSync(envProxy, envContent, 'utf-8');
console.log(`  [env] OK: env.js proxy -> ${mcpEnvUrl}`);
...
const HOOK_REGISTRY = {
  ...
  PreToolUse: [
    { hooks: [{ type: 'command', command: `node "${H}/repetition-detector.js"` }] },
