...
| 項目 | 路徑 |
|------|------|
| MCP Server | `d:\Develop\MCP_NodeServer\` |
| Skills 存放 | `d:\Develop\MCP_NodeServer\Skills\commands\`（有子資料夾分類）|
| Skills 部署 | `~/.claude/commands\`（flat，無子資料夾）|
| Dashboard | `d:\Develop\MCP_NodeServer\docs\dashboard.html` |
| Memory（版控）| `d:\Develop\MCP_NodeServer\memory\` |
| Memory（機敏）| `d:\Develop\MCP_NodeServer\memory\_private\`（.gitignore）|
