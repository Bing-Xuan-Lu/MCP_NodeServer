...
規格書（AxShare）、XD 設計稿頁面、實際網站截圖，一律加 `fullPage: true`。

**Why:** 規格書和 XD 頁面往往比視窗高，不加 fullPage 會漏掉下方的欄位定義、備註說明、業務邏輯。

**How to apply:**

```text
browser_take_screenshot { type: "png", fullPage: true, filename: "u1_spec_home.png" }
```

- 所有規格書頁截圖：`fullPage: true`
- 所有 XD 設計稿截圖：`fullPage: true`
