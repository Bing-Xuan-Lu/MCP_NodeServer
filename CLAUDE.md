...
**Hook 偵測規則**（repetition-detector 20 層 + refactor-advisor 13 項）：
...
| L1.7 | ssh_exec_docker_mysql | ssh_exec 命令含 `docker exec ... mysql` → BLOCK 並指引使用 `set_database({ connection_type: "docker_exec" })` | ❌ |
...
| L2.10b | css_legacy_skill_gate | 寫入頁面層 CSS（路徑含 `/v3/` `/page/` `/page/vN/` 等）首次 BLOCK 提示先跑 `/css_legacy_override`，第二次自動放行；prompt 含「/css_legacy_override」「已查 legacy」「確認覆寫」等字樣即 ack | ❌→放行 |
