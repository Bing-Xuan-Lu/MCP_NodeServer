...
**Hook 偵測規則**：repetition-detector 24 層偵測 + refactor-advisor 14 項 PHP 品質檢查 + 獨立 `agent-coord-stale-contract`、`token-budget-circuit-breaker`（單場 tool call 150 警告 / 250 硬擋）。**完整對照表（每條 ID／觸發條件／行為／放行條件）見 [docs/HOOKS.md](docs/HOOKS.md)**；被 hook 擋到時去那查對應規則。
