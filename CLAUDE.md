...
### Hook 環境變數（repetition-detector.js）

設定 `~/.bashrc` 或 `~/.zshrc`（可選，用於 hook 增強功能）：

| 環境變數 | 值 | 用途 |
| --- | --- | --- |
| `CLAUDE_HOOK_DEBUG` | `1` | 啟用 hook 除錯日誌（`[hook-debug]` 標籤輸出） |
| `CLAUDE_SLACK_WEBHOOK` | `https://hooks.slack.com/...` | Slack webhook URL，用於發送阻擋告警 |
| `CLAUDE_NOTIFY_ON_BLOCK` | `1` | 僅在 block 事件時發送 Slack 通知（預設） |

**Hook 偵測規則**（8層）：

| 層級 | ID | 觸發條件 | 行為 |
| --- | --- | --- | --- |
| L1 | bash_wrong_tool | 用 Bash 做有專用工具的事（docker mysql、cat/grep/find 等） | ⚠️ 警告（部分 block） |
...
| L5 | token_waste_detection | 重複讀檔、低效操作 | 💰 估算 token 浪費 |
