...
│   ├── session-recall-on-prompt.js ← UserPromptSubmit：使用者送出指令時，從 prompt 抽關鍵字（檔名/PascalCase 識別字/URL 段），搜「本專案」歷史找**最相關**那場（非最近那場），注入它動過的檔/失敗呼叫/未完成/片段，讓 Claude 不從零重推、不重踩舊坑。抽不到具體關鍵字或命中<2 則靜默；同場每對話只注入一次。解「session_search 要人手動喊」與「開場按時間撈撈錯場」
...
│   ├── verify-pass-guard.js ← Stop：回合結束掃 assistant 最後訊息，攔「N/N PASS / 全部通過」但無逐行/逐格明細證據的驗證偷懶（合計對 ≠ 明細對；防迴圈：stop_hook_active 放行 + 同宣告 hash 集合去重；無次數上限，每個新的未附明細 PASS 宣告都擋）
...
**Hook 偵測規則**：repetition-detector 26 層偵測（含 L2.4e Grep 找 class 引用建議 find_usages/find_dead_symbols、L2.90 寫 code 前須先查證、L2.89 假設語句寫入硬擋）+ refactor-advisor 14 項 PHP 品質檢查（新生成檔嚴重結構問題會 BLOCK）+ 獨立 `agent-coord-stale-contract`、`token-budget-circuit-breaker`（單場 tool call 150 警告 / 250 硬擋）。**完整對照表（每條 ID／觸發條件／行為／放行條件）見 [docs/HOOKS.md](docs/HOOKS.md)**；被 hook 擋到時去那查對應規則。
