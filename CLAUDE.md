...
│   ├── session-start.js ← SessionStart：對話開場載入記憶與上次摘要 + 浮現 hook 投訴 + 浮現 /lesson 暫存的對話品質教訓（待 /retro lesson 轉化）
...
**Hook 偵測規則**（repetition-detector 24 層 + refactor-advisor 14 項；另有獨立 `agent-coord-stale-contract` PreToolUse hook）：
...
| L1.7 | ssh_exec_docker_exec | ssh_exec 命令含 `docker exec ... mysql/php/python` → BLOCK。mysql 例外：`-e "SHOW/EXPLAIN/CREATE/ALTER/DROP/..."` DDL 放行；尾端 `# mcp-fallback: <reason>` 放行。指引使用 `set_database({connection_type:"docker_exec"})` / `run_php_script({container})` / `run_python_script` | ❌ |
